...
class COCODemo(object):
    ...
    def run_on_opencv_image(self, image):
        ...
        if self.cfg.MODEL.MASK_ON:
            result,dictPosition = self.overlay_mask(result, top_predictions)[:2]
    ...
    #增加增加测试多张图片的方法 
    def run_on_imageList(self,imageList,nameList,jsonFile):
        """
        Created by Fengjia yang 2019.07.19
        predict image as a list
        """
        predictions = []
        for i,image in enumerate(imageList):
            predictions.append(self.compute_prediction(image))
            print("predicting:{}".format(i))
        top_predictions = [self.select_top_predictions(prediction) for prediction in predictions]
        ...
        flag = False
        for i,image in enumerate(imageList):
            print("add_mask:{}".format(i))
            result = image.copy()
            if self.show_mask_heatmaps:
                return self.create_mask_montage(result, top_predictions[i])
            # 不显示算法原有的bounding box 
            #result = self.overlay_boxes(result, top_predictions[i])
            if self.cfg.MODEL.MASK_ON:
                #result,dictPosition,flag= self.overlay_mask(result, top_predictions[i])
                result,dictPosition,flag= self.select_the_connected_mask(result, top_predictions[i])
                if flag:
                    dictPosition["name"] = nameList[i]
            ...
            # 统计未能实现分割的样本
            if not flag:
                errorList.append(nameList[i])
            else:
                result = self.overlay_class_names(result, top_predictions[i])
                resultList.append(result)
                successList.append(nameList[i])
        # 将计算结果存入json文件
        json.dump(dictionaryList,jsonFile)
        return resultList,errorList,successList
    ...
    def select_the_connected_mask(self,image,predictions):
        ...
        for mask, color, score, label in zip(masks, colors, scores, labels):
            ...
            if not contours or self.CATEGORIES[label] in del_set:
                continue
            # 剔除一些低置信度的人类和鸟类的轮廓 
            if (self.CATEGORIES[label] == 'person' or self.CATEGORIES[label] == 'bird') and score < 0.5:
                continue
            ...
            if score >= 0.7:
                sub_dict = {}
                contours = [np.reshape(unit,(unit.shape[0],-1)) for unit in contours]
                if len(contours)>1:            
                    contours_array = np.concatenate((tuple([unit for unit in contours])), axis = 0)
                else:
